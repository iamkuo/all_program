    ...
    for j in range(4):
        ...
        for i in range(1,con_win):
            ...
            if ttr >= 0 and ttc >= 0 and ttc < len(cb[0]):
                ...
                break
def gameFinish():
    global p,w_width,w_height,show_i,times,left_x,left_y,right_x,right_y
...
def mousePressed():
    ...
    if going == 1:
        ...
        if (mouseX>left_x-((show_i/2)*0.36)) and (mouseX<left_x+((show_i/2)*0.36)) and (mouseY>left_y-((show_i/2)*0.25)) and (mouseY<left_y+((show_i/2)*0.25)):
            #playAgain() == True:
            p = 0
            strokeWeight(3)
            background(back_c)
