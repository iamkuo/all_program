...
func get_spawn_point(team:int, lane:int) -> Vector2:
    var name = ""
    if team == 0:
        name = lane == 0 ? "Spawn_L_Top" : lane == 1 ? "Spawn_L_Mid" : "Spawn_L_Bot"
    else:
        name = lane == 0 ? "Spawn_R_Top" : lane == 1 ? "Spawn_R_Mid" : "Spawn_R_Bot"
    var p = spawn_points.get_node(name)
    if p:
...
func ai_play_card():
    var cards = [ preload("res://scenes/cards/Archer.tscn"), preload("res://scenes/cards/Tank.tscn"), preload("res://scenes/cards/Speeder.tscn") ]
